"""
ridge.py — 2-D Ridge subgradient optimizer
=============================================================

Helper for running subgradient descent on a 2-D ridge objective:

    0.5 * mean((y - X @ w)**2) + factor * ||w||_2

Contents:
- RidgeSubgradientOptimizer: builds MSE + L2 subgradients, runs optimization via
  SubgradientDescent, and provides contour & surface visualizations.
...
class RidgeSubgradientOptimizer:
    """
    Subgradient descent helper specialized for 2-D ridge regression weight space.
    ...
    def l2_subgradient(self, w: np.ndarray) -> np.ndarray:
        norm = np.linalg.norm(w)
        if norm < 1e-12:
            # Subgradient can be any vector with ||g|| <= 2 when w == 0.
            return np.zeros_like(w)
        return 2.0 * w / norm

    def objective(self, w: np.ndarray) -> float:
        return self.mse_loss(w) + self.factor * np.linalg.norm(w, 2)

    def objective_subgradient(self, w: np.ndarray) -> np.ndarray:
        return self.mse_grad(w) + self.factor * self.l2_subgradient(w)
    ...
    def plot_loss_contours(
        ...
        ax.set_xlabel("w0")
        ax.set_ylabel("w1")
        ax.set_title("Ridge Objective Contours")
        plt.show()
    ...
    def plot_loss_surface(
        ...
        ax.set_title("Ridge Objective Surface")
